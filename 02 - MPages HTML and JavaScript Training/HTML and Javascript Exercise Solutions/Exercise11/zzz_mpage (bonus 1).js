	...
	algyInfo.onreadystatechange = function () {
		if (algyInfo.readyState == 4 && algyInfo.status == 200) {
			...
			for (var i=0, aLen=all_var.length;i<aLen;i++){
				...
				tableBody.push(
						"<tr",OddRow,">",
                    		"<td class='col1'><a href=\"javascript:addAllergy(",all_var2[2].text,")\">",all_var2[0].text,"</a></td>",
...
function addAllergy( algId ) {
	var paramString =  "$PAT_PERSONID$|$VIS_ENCNTRID$|"+algId+"|0|||0||0|0";
